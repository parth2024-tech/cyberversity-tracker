...
class SlackDelivery(BaseDelivery):
    ...
    async def send_digest(
        ...
        try:
            ...
            return DeliveryResult(
                success=True, channel=self.channel_name, message="Slack message sent"
            )
    ...
    async def send_alert(self, entry: Entry, analysis: Analysis) -> DeliveryResult:
        ...
        try:
            blocks = [
                {
                    ...
                    "text": {
                        "type": "plain_text",
                        "text": "🚨 High-Velocity Threat Alert",
                    },
                },
                {
                    "type": "section",
            ...
            return DeliveryResult(
                success=True, channel=self.channel_name, message="Slack alert sent"
            )
        except Exception as e:
            return DeliveryResult(
                success=False, channel=self.channel_name, error=str(e)
        ...
        blocks = [
            {
                ...
                "text": {
                    "type": "plain_text",
                    "text": f"📋 {digest.schedule.title()} Digest",
                },
            },
            {
                "type": "context",
                "elements": [
                    {
                        "type": "mrkdwn",
                        "text": f"Period: {digest.period_start.strftime('%Y-%m-%d')} to {digest.period_end.strftime('%Y-%m-%d')} | Total: {digest.total_entries} entries",
                    }
                ],
            },
            {"type": "divider"},
                ...
                blocks.append(
                    {
                        ...
                        "elements": [
                            {
                                "type": "mrkdwn",
                                "text": f"... and {len(cat_entries) - 5} more entries",
                            }
                        ],
                    }
                )
