...
def _clean_entry_summary(
    raw_summary: str | None, entry: Any, cleaned_title: str | None = None
) -> str:
...
async def query_serialized_entries(
    ...
    filters = EntryFilters(
        ...
        country=country
        if country and country != "all" and isinstance(country, str)
        else None,
    )
    ...
    for e in entries:
        ...
        if e.analysis:
            ...
            analysis_dict = {
                ...
                "capability_summary": e.analysis.risk_assessment
                if is_ai_innov
                else None,
            ...
            fallback_focus = (
                "Trending Open-Source Developer Repository"
                if cat_val == "github_trending"
                else "Frontier AI Foundation Weights & Model Architecture"
                if cat_val == "ai_models"
                else "Academic Research & ArXiv Paper"
                if cat_val == "ai_research"
                else "AI Framework & Developer Infrastructure"
                if cat_val == "cyber_tools"
            ...
            analysis_dict = {
                ...
                "capability_summary": "Ecosystem milestone in global machine learning and autonomous systems."
                if is_ai_innov
                else None,
                "actionable_insight": "Review official repository and documentation for deployment."
                if is_ai_innov
                else None,
            }
        ...
        is_imp = bool(
            meta_dict.get("is_important", False)
            or meta_dict.get("is_saved", False)
            or meta_dict.get("is_pinned", False)
        )
        ...
        serialized_entries.append(
            {
                ...
                "severity": severity
                or (analysis_dict["severity_index"] if analysis_dict else 0),
                "blast_radius": blast_radius
                or (analysis_dict["blast_radius_score"] if analysis_dict else 0),
                "threat_velocity": (
                    analysis_dict["threat_velocity"] if analysis_dict else 0
                ),
                "mitigation": mitigation
                or (analysis_dict["mitigation"] if analysis_dict else None),
                "attack_archetype": attack_archetype
                or (analysis_dict["attack_archetype"] if analysis_dict else None),
                "is_ai_innovation": analysis_dict.get("is_ai_innovation", False)
                if analysis_dict
                else False,
                "innovation_score": analysis_dict.get("innovation_score", 0)
                if analysis_dict
                else 0,
                "tech_focus": analysis_dict.get("tech_focus")
                if analysis_dict
                else None,
                "capability_summary": analysis_dict.get("capability_summary")
                if analysis_dict
                else None,
                "actionable_insight": analysis_dict.get("actionable_insight")
                if analysis_dict
                else None,
            }
        )
...
class ExportPdfRequest(BaseModel):
    ids: list[str] | None = Field(
        default=None, description="Specific intelligence IDs to include"
    )
    title: str | None = Field(
        default="AetherGuard Global AI & Technology Intelligence Dossier"
    )
    subtitle: str | None = Field(
        default="Frontier AI Systems & Technology Intelligence Briefing"
    )
...
async def export_selected_entries_pdf(payload: ExportPdfRequest):
    ...
    if payload.ids:
        ...
        all_entries, _ = await query_serialized_entries(
            limit=max(500, len(payload.ids) * 2), offset=0, sort_by="newest"
        )
        filtered = [e for e in all_entries if e["id"] in target_ids]
    else:
        all_entries, _ = await query_serialized_entries(
            limit=150, offset=0, sort_by="velocity"
        )
    ...
    pdf_bytes = PdfExportService.generate_dossier_pdf(
        entries=filtered,
        title=payload.title
        or "AetherGuard Global AI & Technology Intelligence Dossier",
        subtitle=payload.subtitle
        or f"Executive AI & Technology Brief ({len(filtered)} items)",
    )
    ...
    async with SqlAlchemyUnitOfWork() as uow:
        ...
        if entry.analysis:
            analysis_dict = {
                ...
                "model": entry.analysis.model.value
                if hasattr(entry.analysis.model, "value")
                else str(entry.analysis.model),
                ...
                "capability_summary": entry.analysis.risk_assessment
                if is_ai_cat
                else None,
