...
class DigestBase(BaseModel):
    ...
    entries_by_category: dict[str, list] = Field(
        default_factory=dict, description="Entries grouped by category"
    )
    delivery_channels: list[str] = Field(
        default_factory=list, description="Delivery channels used"
    )
...
class DigestSendRequest(BaseModel):
    ...
    schedule: str = Field(
        ..., pattern="^(daily|weekly)$", description="Digest schedule"
    )
