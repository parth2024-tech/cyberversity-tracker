...
class DesktopStatusWidget:
    ...
    async def run(self) -> None:
        self._running = True
        logger.info(
            f"Starting AetherGuard Desktop Status Bar Widget connecting to {self.ws_url}..."
        )
        while self._running:
            try:
                async with websockets.connect(self.ws_url) as websocket:
                    ...
                    async for message in websocket:
                        ...
                        if msg_type == "connected":
                            ...
                            logger.info(
                                f"[STATUS BAR] Online | Total Entries: {stats.get('total_entries', 0)} | Threat Velocity Avg: {stats.get('avg_velocity', 0)}"
                            )
                        elif msg_type == "triage_queue_updated":
                            q_data = data.get("data", {})
                            logger.info(
                                f"[STATUS BAR ALERT] Triage Queue Updated: {q_data.get('queue_size', 0)} pending LLM analysis."
                            )
            except Exception as e:
                logger.warning(
                    f"Status Bar Widget connection drop ({e}). Reconnecting in 5 seconds..."
                )
