...
"""
    ...
    def _render_pdf(
        ...
        def make_table(headers: list, rows: list, col_widths: list) -> Table:
            ...
            data.extend([[Paragraph(str(cell), item_summary) for cell in row] for row in rows])
        ...
        for item in categorized.get("trending_repos", [])[:5]:
            story.extend(render_article_card(item, tag_color="#0891b2", tag_label="TRENDING REPO", stat_label="VEL"))
        story.append(PageBreak())
        ...
        for item in categorized.get("ai_models", [])[:5]:
            story.extend(render_article_card(item, tag_color="#7c3aed", tag_label="FRONTIER MODEL", stat_label="IMPACT"))
        story.append(PageBreak())
        ...
        for item in categorized.get("ai_research", [])[:6]:
            story.extend(render_article_card(item, tag_color="#4338ca", tag_label="AI RESEARCH", stat_label="VEL"))
        story.append(PageBreak())
        ...
        for item in categorized.get("ai_tools", [])[:6]:
            story.extend(render_article_card(item, tag_color="#0d9488", tag_label="AI TOOL", stat_label="VEL"))
        story.append(PageBreak())
        ...
        for item in categorized.get("sovereign_ai", [])[:6]:
            ...
            story.extend(render_article_card(item, tag_color="#b91c1c", tag_label=f"SOVEREIGN [{c_code}]", stat_label="VEL"))
        story.append(PageBreak())
        ...
        for item in categorized.get("ai_hardware", [])[:5]:
            story.extend(render_article_card(item, tag_color="#d97706", tag_label="AI HARDWARE", stat_label="VEL"))
        story.append(PageBreak())
        ...
        for item in categorized.get("autonomous_agents", [])[:5]:
            story.extend(render_article_card(item, tag_color="#059669", tag_label="AGENT SYSTEM", stat_label="VEL"))
        story.append(PageBreak())
        ...
        for item in categorized.get("overflow", [])[:8]:
            story.extend(render_article_card(item, tag_color="#64748b", tag_label="OVERFLOW", stat_label="VEL"))
