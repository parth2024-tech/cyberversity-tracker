...
async def test_api_rate_limiting():
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://testserver") as client:
        ...
        print("✅ API rate limiter successfully capped requests at 60/min and returned 429 without crash.")
...
async def test_websocket_backpressure_real():
    ...
    assert slow_client not in cm.active_connections, "Slow client should have been evicted"
    slow_client.close.assert_called_once_with(code=1008)

    print(f"✅ Stalled WebSocket client disconnected with code 1008 after {duration:.2f}s (<= 2.5s).")
...
def test_real_database_migration():
    ...
    print("✅ Real database schema verified: WAL mode active, is_purged & purged_at present with data intact.")
...
async def test_github_scrape_health_check():
    ...
    with patch.object(fetcher, "_fetch_raw_scraping", new_callable=AsyncMock) as mock_scraping, \
        ...
        mock_api.return_value = [
            {"title": "fallback/repo1", "url": "https://github.com/fallback/repo1", "summary": "LLM tool"}
        ]

        result = await fetcher._fetch_raw()

        mock_api.assert_called_once()
        print(f"Scrape failure successfully detected, fallback called: {mock_api.called}")
        print(f"Fallback entries returned: {len(result)}")
        print("✅ Scrape health check triggered fallback when DOM lacked expected elements.")
...
async def test_concurrency_timing():
    ...
    assert duration_12 < duration_4, "Adaptive concurrency must complete faster than fixed 4"
