...
const DEFAULT_PALETTE_ACTIONS = [
  ...
  { id: 'action-theatre-china', title: 'Switch Theatre to China Intel & AI', subtitle: 'Filter DeepSeek, Qwen, Zhipu & sovereign China AI labs', icon: 'globe', badge: '🇨🇳 CHINA', type: 'action', run: () => typeof setRegionFilter === 'function' ? setRegionFilter('china') : null },
  { id: 'action-theatre-india', title: 'Switch Theatre to India & South Asia', subtitle: 'Filter Indian AI startups, Sarvam, Krutrim & IIT AI research', icon: 'globe', badge: '🇮🇳 INDIA', type: 'action', run: () => typeof setRegionFilter === 'function' ? setRegionFilter('south_asia') : null },
  { id: 'action-theatre-middle-east', title: 'Switch Theatre to Middle East (Israel & UAE)', subtitle: 'Filter TII Falcon, Middle East AI hubs & regional research', icon: 'globe', badge: '🇮🇱 🇦🇪 MIDEAST', type: 'action', run: () => typeof setRegionFilter === 'function' ? setRegionFilter('middle_east') : null },
  { id: 'action-theatre-apac', title: 'Switch Theatre to Asia-Pacific (Japan, Korea, Taiwan)', subtitle: 'Filter TSMC, Samsung AI, Preferred Networks & robotics', icon: 'globe', badge: '🇯🇵 🇰🇷 APAC', type: 'action', run: () => typeof setRegionFilter === 'function' ? setRegionFilter('apac') : null },
  { id: 'action-export-pdf', title: 'Export AI Intelligence PDF Dossier', subtitle: 'Download executive multi-page PDF briefing report', icon: 'file-down', badge: 'PDF', type: 'action', run: () => typeof exportPdfReport === 'function' ? exportPdfReport() : null },
  { id: 'action-auto-triage', title: 'Run Autonomous Local LLM Triage', subtitle: 'Queue top models & papers for local Ollama (llama3.1:8b) deep analysis', icon: 'sparkles', badge: 'AI TRIAGE', type: 'action', run: () => typeof triggerTriageBackfill === 'function' ? triggerTriageBackfill() : null },
  { id: 'action-scan', title: 'Trigger Real-Time Intelligence Sweep', subtitle: 'Ingest latest arXiv papers, models & trending GitHub repos', icon: 'refresh-cw', badge: 'RADAR', type: 'action', run: () => typeof triggerScan === 'function' ? triggerScan() : null },
  { id: 'action-3d', title: 'Toggle 3D Holographic Globe Radar', subtitle: 'View or hide Three.js spatial AI intelligence observatory', icon: 'globe', badge: '3D WEBGL', type: 'action', run: () => typeof toggle3DGlobe === 'function' ? toggle3DGlobe() : null },
  { id: 'action-h24', title: 'Filter: Last 24 Hours Only', subtitle: 'Show emerging models & breakthroughs discovered in past 24h', icon: 'clock', badge: '24H', type: 'action', run: () => typeof setTimeHorizon === 'function' ? setTimeHorizon(24) : null },
  { id: 'action-pinned', title: 'Open Active Investigation Pinboard', subtitle: 'View all bookmarked & pinned research files', icon: 'star', badge: 'PINBOARD', type: 'action', run: () => typeof setCategory === 'function' ? setCategory('pinned') : null },
  { id: 'action-export-md', title: 'Export AI Intelligence Markdown Brief', subtitle: 'Download formatted report for Notion/Obsidian/GitHub', icon: 'file-text', badge: 'REPORT', type: 'action', run: () => typeof exportMarkdownReport === 'function' ? exportMarkdownReport() : null },
  { id: 'action-export-stix', title: 'Export AI Intelligence JSON Bundle', subtitle: 'Download JSON dataset for research ingestion', icon: 'shield', badge: 'JSON', type: 'action', run: () => typeof exportStixJson === 'function' ? exportStixJson() : null },
  { id: 'action-watchlist', title: 'Manage Framework Watchlists', subtitle: 'Configure keyword rules and alerts', icon: 'target', badge: 'RULES', type: 'action', run: () => typeof openWatchlistModal === 'function' ? openWatchlistModal() : null },
